...
function createISGCommands(
    ...
    adapter.setObjectNotExists(
        ...
        function () {
            adapter.setState(`${strGroup}.${valTag}`, { val: valValue, ack: true });
        },
    );
}
                                        ...
                                        .each(function (_k, elem) {
                                            ...
                                            if (!(idCommand.match(/aval/) || idCommand.match(/info/))) {
                                                if (idCommand.match(/[0-9]s/)) {
                                                    ...
                                                    statesCommand += `"${$(elem).attr('value')}":"${$(elem)
                                                        .next()
                                                        .text()}"`;
                                                } else {
                            ...
                            if (scriptValues) {
                                ...
                                if (idCommand) {
                                    ...
                                    createISGCommands(
                                        ...
                                        unitCommand,
                                        'state',
                                        parseFloat(valCommand[1].replace(',', '.').replace(' ', '')),
                                        '',
                                        parseFloat(minCommand[1].replace(',', '.').replace(' ', '')),
                                        parseFloat(maxCommand[1].replace(',', '.').replace(' ', '')),
                                    );
                                }
                            }
