...
### Adapter-Specific Context: Stiebel-ISG

This adapter interfaces with **Stiebel-Eltron/Tecalor Internet Service Gateway (ISG)** devices. The ISG is a web-based gateway that provides access to heat pump systems and enables remote monitoring and control.

#### Key Features:
- **Device Communication**: Communicates with ISG devices via HTTP/HTTPS requests
- **Web Scraping**: Uses Cheerio to parse HTML responses from the ISG web interface
- **Cookie Management**: Maintains session cookies using tough-cookie for authenticated requests
- **Polling**: Regularly polls the ISG for status updates and values
- **Command Execution**: Allows sending commands to control the heat pump system
- **Multi-language Support**: Supports multiple languages through i18n translations

#### Configuration Parameters:
- `isgUser`: Username for ISG authentication
- `isgPassword`: Password for ISG authentication
- `isgAddress`: IP address or hostname of the ISG device
- `isgIntervall`: Polling interval for reading values (in seconds)
- `isgCommandIntervall`: Interval for reading command paths (in seconds)
- `isgUmlauts`: Handling of German umlauts in state names
- `isgExpert`: Enable expert mode for additional parameters
- `isgCommandPaths`: Semicolon-separated list of command paths to monitor
- `isgValuePaths`: Semicolon-separated list of value paths to read
- `isgStatusPaths`: Semicolon-separated list of status paths to monitor
...
- The ISG device may not be publicly accessible, so tests should be designed to work with mock data
- Authentication is handled via web forms and session cookies
- The adapter implements a reboot command for the ISG device
- State changes trigger command execution on the ISG device
- Resource cleanup is critical - ensure all intervals are cleared in the unload method
